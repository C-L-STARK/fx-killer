      ...
      <section id="what-is-risk-reward" className="mb-16 scroll-mt-20">
        ...
        <div className="space-y-4 text-gray-300 dark:text-gray-300 leading-relaxed">
          ...
          <div className="p-6 bg-gray-900 text-gray-300 border-2 border-gray-800">
            <h3 className="text-xl font-bold mb-4 text-white">{isZh ? '风险回报比基础公式' : 'Risk-Reward Ratio Basic Formula'}</h3>
            ...
            <ul className="space-y-2">
              <li className="text-gray-300">
                <strong className="text-white">1:1</strong> - {isZh ? '冒险$100赚$100（止损30点，止盈30点）' : 'Risk $100 to earn $100 (stop 30 pips, target 30 pips)'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">1:2</strong> - {isZh ? '冒险$100赚$200（止损30点，止盈60点）' : 'Risk $100 to earn $200 (stop 30 pips, target 60 pips)'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">1:3</strong> - {isZh ? '冒险$100赚$300（止损30点，止盈90点）' : 'Risk $100 to earn $300 (stop 30 pips, target 90 pips)'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">1:5</strong> - {isZh ? '冒险$100赚$500（止损30点，止盈150点）' : 'Risk $100 to earn $500 (stop 30 pips, target 150 pips)'}
              </li>
            </ul>
          </div>
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <p className="font-bold text-white">{isZh ? '示例1：EUR/USD日内交易' : 'Example 1: EUR/USD Day Trade'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '入场价：1.1050' : 'Entry: 1.1050'}</li>
                  <li className="text-gray-300">{isZh ? '止损价：1.1020（30点）' : 'Stop: 1.1020 (30 pips)'}</li>
                  <li className="text-gray-300">{isZh ? '止盈价：1.1110（60点）' : 'Target: 1.1110 (60 pips)'}</li>
                  <li className="text-gray-300">{isZh ? '潜在亏损：30点 = $300（1手）' : 'Potential loss: 30 pips = $300 (1 lot)'}</li>
                  <li className="text-gray-300">{isZh ? '潜在盈利：60点 = $600（1手）' : 'Potential profit: 60 pips = $600 (1 lot)'}</li>
              ...
              <div className="space-y-3">
                <p className="font-bold text-white">{isZh ? '示例2：GBP/JPY波段交易' : 'Example 2: GBP/JPY Swing Trade'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '入场价：185.00' : 'Entry: 185.00'}</li>
                  <li className="text-gray-300">{isZh ? '止损价：184.50（50点）' : 'Stop: 184.50 (50 pips)'}</li>
                  <li className="text-gray-300">{isZh ? '止盈价：186.50（150点）' : 'Target: 186.50 (150 pips)'}</li>
                  <li className="text-gray-300">{isZh ? '潜在亏损：50点 = $325（1手）' : 'Potential loss: 50 pips = $325 (1 lot)'}</li>
                  <li className="text-gray-300">{isZh ? '潜在盈利：150点 = $975（1手）' : 'Potential profit: 150 pips = $975 (1 lot)'}</li>
                  <li className="font-bold text-white">
          ...
          <div className="p-6 bg-gray-900 text-gray-300 border-2 border-gray-800">
            ...
            <ul className="space-y-3">
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '1. 决定盈亏平衡胜率：' : '1. Determines Breakeven Win Rate:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '2. 放大策略优势：' : '2. Amplifies Strategy Edge:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '3. 降低心理压力：' : '3. Reduces Psychological Pressure:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '4. 复利增长的基础：' : '4. Foundation of Compound Growth:'}</strong>{' '}
                {isZh
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '方法1：点数计算法' : 'Method 1: Pip Calculation'}</p>
                <p className="text-sm mb-2">
                  {isZh
                    ? '基于止损和止盈的点数差计算R:R，最直观简单。'
                    : 'Calculate R:R based on pip difference between stop and target, most intuitive.'}
                </p>
                <div className="font-mono text-sm bg-gray-50 dark:bg-black p-2 text-white">
                  {isZh
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '方法2：金额计算法' : 'Method 2: Dollar Amount Calculation'}</p>
                <p className="text-sm mb-2">
                  {isZh
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '方法3：R倍数法' : 'Method 3: R Multiple Method'}</p>
                <p className="text-sm mb-2">
                  {isZh
              ...
              <table className="w-full border-2 border-gray-800 text-sm">
                <thead>
                  <tr className="bg-gray-900 text-gray-300">
                    <th className="border border-gray-800 p-3 text-left">{isZh ? '场景' : 'Scenario'}</th>
                    <th className="border border-gray-800 p-3 text-left">{isZh ? '推荐R:R' : 'Recommended R:R'}</th>
                    <th className="border border-gray-800 p-3 text-left">{isZh ? '原因' : 'Reason'}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '强势趋势突破' : 'Strong Trend Breakout'}</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">1:3-1:5</td>
                  ...
                  <tr className="bg-gray-50 dark:bg-[#0a0a0a]">
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '支撑/阻力反弹' : 'Support/Resistance Bounce'}</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">1:2-1:3</td>
                    <td className="border border-gray-800 p-3 text-gray-300">
                      {isZh ? '反弹空间有限，目标至下一个关键位' : 'Bounce space limited, target to next key level'}
                    </td>
                  </tr>
                  <tr>
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '震荡区间交易' : 'Range Trading'}</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">1:1.5-1:2</td>
                    <td className="border border-gray-800 p-3 text-gray-300">
                      {isZh ? '区间有限，目标为区间另一端' : 'Range limited, target is other end of range'}
                    </td>
                  </tr>
                  <tr className="bg-gray-50 dark:bg-[#0a0a0a]">
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '新闻前抢跑' : 'News Front-Running'}</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">1:1-1:1.5</td>
                    <td className="border border-gray-800 p-3 text-gray-300">
                      {isZh ? '风险高，快进快出，小目标即可' : 'High risk, quick in-out, small target sufficient'}
                    </td>
                  </tr>
                  <tr>
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '回调入场（趋势延续）' : 'Pullback Entry (Trend Continuation)'}</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">1:2.5-1:4</td>
                    <td className="border border-gray-800 p-3 text-gray-300">
