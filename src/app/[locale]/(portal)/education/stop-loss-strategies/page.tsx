      ...
      <section id="why-stop-loss" className="mb-16 scroll-mt-20">
        ...
        <div className="space-y-4 text-gray-300 dark:text-gray-300 leading-relaxed">
          ...
          <div className="p-6 bg-gray-900 text-gray-300 border-2 border-gray-800">
            <h3 className="text-xl font-bold mb-4 text-white">{isZh ? '为什么必须使用止损？' : 'Why Must Use Stop-Loss?'}</h3>
            <ul className="space-y-3">
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '1. 防止灾难性亏损：' : '1. Prevent Catastrophic Loss:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '2. 消除情绪决策：' : '2. Eliminate Emotional Decisions:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '3. 保护交易资本：' : '3. Protect Trading Capital:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '4. 验证交易假设：' : '4. Validate Trading Hypothesis:'}</strong>{' '}
                {isZh
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="space-y-3 text-gray-300 dark:text-gray-300">
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '实施步骤：' : 'Implementation Steps:'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '1. 根据货币对平均波动性确定固定点数（EUR/USD: 20-30点，GBP/JPY: 40-60点）' : '1. Determine fixed pips based on pair average volatility (EUR/USD: 20-30 pips, GBP/JPY: 40-60 pips)'}</li>
                  <li className="text-gray-300">{isZh ? '2. 开仓时，在入场价基础上加减固定点数设置止损' : '2. When opening, add/subtract fixed pips from entry price to set stop'}</li>
                  <li className="text-gray-300">{isZh ? '3. 根据止损点数反推仓位大小，确保风险固定（如$200）' : '3. Reverse calculate position size from stop pips, ensure fixed risk (e.g., $200)'}</li>
                  <li className="text-gray-300">{isZh ? '4. 止损一旦设定，绝不移动（除非朝盈利方向）' : '4. Once stop set, never move (unless toward profit)'}</li>
                </ul>
              </div>
              <div className="grid md:grid-cols-2 gap-4 mt-3">
                <div className="p-4 bg-green-50 dark:bg-green-900/20 border-2 border-green-600">
                  ...
                  <ul className="list-disc pl-6 space-y-1 text-sm">
                    <li className="text-gray-300">{isZh ? '简单易执行，无需判断市场结构' : 'Simple to execute, no need to judge market structure'}</li>
                    <li className="text-gray-300">{isZh ? '风险一致，便于资金管理' : 'Consistent risk, easy for money management'}</li>
                    <li className="text-gray-300">{isZh ? '适合自动化交易（EA）' : 'Suitable for automated trading (EA)'}</li>
                    <li className="text-gray-300">{isZh ? '回测数据准确，易于优化' : 'Accurate backtest data, easy to optimize'}</li>
                  </ul>
                </div>
                <div className="p-4 bg-red-50 dark:bg-red-900/20 border-2 border-red-600">
                  ...
                  <ul className="list-disc pl-6 space-y-1 text-sm">
                    <li className="text-gray-300">{isZh ? '忽略市场结构，可能止损位不合理' : 'Ignores market structure, stop position may be unreasonable'}</li>
                    <li className="text-gray-300">{isZh ? '高波动期止损太小，低波动期止损太大' : 'During high volatility stop too small, low volatility stop too large'}</li>
                    <li className="text-gray-300">{isZh ? '容易在整数关口被"猎杀"' : 'Easily "hunted" at round numbers'}</li>
                    <li className="text-gray-300">{isZh ? '不同时间框架需要不同点数，但固定止损无法适应' : 'Different timeframes need different pips, but fixed stop cannot adapt'}</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="space-y-3 text-gray-300 dark:text-gray-300">
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                ...
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '做多EUR/USD，入场价1.1000' : 'Long EUR/USD, entry 1.1000'}</li>
                  <li className="text-gray-300">{isZh ? '百分比止损：0.5%' : 'Percentage stop: 0.5%'}</li>
                  <li className="text-gray-300">{isZh ? '止损价 = 1.1000 × (1 - 0.5%) = 1.0945' : 'Stop price = 1.1000 × (1 - 0.5%) = 1.0945'}</li>
                  <li className="text-gray-300">{isZh ? '止损距离 = 1.1000 - 1.0945 = 55点' : 'Stop distance = 1.1000 - 1.0945 = 55 pips'}</li>
                </ul>
              </div>
              <p className="text-sm">
                <strong className="text-white">{isZh ? '适用场景：' : 'Applicable Scenarios:'}</strong>{' '}
                {isZh
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="space-y-3 text-gray-300 dark:text-gray-300">
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '实施规则：' : 'Implementation Rules:'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '剥头皮交易：持仓5-15分钟后平仓' : 'Scalping: close after 5-15 minutes'}</li>
                  <li className="text-gray-300">{isZh ? '日内短线：持仓2-4小时后平仓' : 'Intraday: close after 2-4 hours'}</li>
                  <li className="text-gray-300">{isZh ? '日内波段：收盘前30分钟平仓（避免隔夜）' : 'Intraday swing: close 30 min before market close (avoid overnight)'}</li>
                  <li className="text-gray-300">{isZh ? '波段交易：持仓3-5天后平仓' : 'Swing trading: close after 3-5 days'}</li>
                </ul>
              </div>
              <p className="text-sm">
              ...
              <p className="text-sm">
                <strong className="text-white">{isZh ? '缺点：' : 'Disadvantages:'}</strong>{' '}
                {isZh
        ...
        <div className="space-y-6 text-gray-300 dark:text-gray-300 leading-relaxed">
          ...
          <div className="p-6 bg-gray-900 text-gray-300 border-2 border-gray-800">
            ...
            <div className="p-4 bg-gray-50 dark:bg-black text-white">
              <p className="font-bold mb-2 text-white">{isZh ? 'ATR止损公式：' : 'ATR Stop Formula:'}</p>
              ...
              <ul className="list-disc pl-6 space-y-1 text-sm">
                <li className="text-gray-300">{isZh ? '保守型：1倍ATR（适合剥头皮、短线）' : 'Conservative: 1x ATR (for scalping, short-term)'}</li>
                <li className="text-gray-300">{isZh ? '平衡型：1.5-2倍ATR（适合日内交易）' : 'Balanced: 1.5-2x ATR (for day trading)'}</li>
                <li className="text-gray-300">{isZh ? '宽松型：2-3倍ATR（适合波段交易）' : 'Loose: 2-3x ATR (for swing trading)'}</li>
              </ul>
            </div>
          </div>
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <p className="font-bold text-white">{isZh ? '示例1：EUR/USD日内交易' : 'Example 1: EUR/USD Day Trading'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? 'H1图表，ATR(14) = 12点' : 'H1 chart, ATR(14) = 12 pips'}</li>
                  <li className="text-gray-300">{isZh ? '使用2倍ATR止损 = 12 × 2 = 24点' : 'Use 2x ATR stop = 12 × 2 = 24 pips'}</li>
                  <li className="text-gray-300">{isZh ? '做多入场价1.1050' : 'Long entry 1.1050'}</li>
                  <li className="text-gray-300">{isZh ? '止损价 = 1.1050 - 0.0024 = 1.1026' : 'Stop price = 1.1050 - 0.0024 = 1.1026'}</li>
                  <li className="text-gray-300">{isZh ? '账户$10,000，风险2% = $200' : 'Account $10,000, risk 2% = $200'}</li>
                  <li className="text-gray-300">{isZh ? '仓位 = $200 / (24点 × $10) = 0.83手' : 'Position = $200 / (24 pips × $10) = 0.83 lots'}</li>
                </ul>
              </div>
              <div className="space-y-3">
                <p className="font-bold text-white">{isZh ? '示例2：GBP/JPY波段交易' : 'Example 2: GBP/JPY Swing Trading'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? 'H4图表，ATR(14) = 65点' : 'H4 chart, ATR(14) = 65 pips'}</li>
                  <li className="text-gray-300">{isZh ? '使用2.5倍ATR止损 = 65 × 2.5 = 162.5点' : 'Use 2.5x ATR stop = 65 × 2.5 = 162.5 pips'}</li>
                  <li className="text-gray-300">{isZh ? '做空入场价185.50' : 'Short entry 185.50'}</li>
                  <li className="text-gray-300">{isZh ? '止损价 = 185.50 + 1.625 = 187.125' : 'Stop price = 185.50 + 1.625 = 187.125'}</li>
                  <li className="text-gray-300">{isZh ? '账户$10,000，风险2% = $200' : 'Account $10,000, risk 2% = $200'}</li>
                  <li className="text-gray-300">{isZh ? '仓位 = $200 / (162.5点 × $6.5) ≈ 0.19手' : 'Position = $200 / (162.5 pips × $6.5) ≈ 0.19 lots'}</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-2 border-gray-800 text-sm">
                ...
                <tbody>
                  <tr>
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '剥头皮' : 'Scalping'}</td>
                    <td className="border border-gray-800 p-3 text-gray-300">M1-M5</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">0.8-1.2x</td>
                  ...
                  <tr className="bg-gray-50 dark:bg-[#0a0a0a]">
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '短线日内' : 'Intraday Short'}</td>
                    <td className="border border-gray-800 p-3 text-gray-300">M15-M30</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">1.5-2x</td>
                    <td className="border border-gray-800 p-3 text-gray-300">
                      {isZh ? '适应市场噪音' : 'Adapt to market noise'}
                    </td>
                  </tr>
                  <tr>
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '日内波段' : 'Intraday Swing'}</td>
                    <td className="border border-gray-800 p-3 text-gray-300">H1</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">2-2.5x</td>
                    <td className="border border-gray-800 p-3 text-gray-300">
                      {isZh ? '平衡噪音与趋势' : 'Balance noise and trend'}
                    </td>
                  </tr>
                  <tr className="bg-gray-50 dark:bg-[#0a0a0a]">
                    <td className="border border-gray-800 p-3 text-gray-300">{isZh ? '波段交易' : 'Swing Trading'}</td>
                    <td className="border border-gray-800 p-3 text-gray-300">H4-D1</td>
                    <td className="border border-gray-800 p-3 font-bold text-gray-300 text-white">2.5-3x</td>
                    <td className="border border-gray-800 p-3 text-gray-300">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="p-6 bg-green-50 dark:bg-green-900/20 border-2 border-green-600">
              ...
              <ul className="list-disc pl-6 space-y-2 text-sm">
                <li className="text-gray-300">{isZh ? '自动适应市场波动性，高波动时止损宽、低波动时止损窄' : 'Auto-adapts to market volatility, wider stop in high volatility, tighter in low'}</li>
                <li className="text-gray-300">{isZh ? '降低被正常波动止损的概率' : 'Reduces probability of being stopped by normal volatility'}</li>
                <li className="text-gray-300">{isZh ? '适用于所有货币对和时间框架' : 'Applicable to all pairs and timeframes'}</li>
                <li className="text-gray-300">{isZh ? '结合仓位计算，风险始终一致' : 'Combined with position sizing, risk always consistent'}</li>
                <li className="text-gray-300">{isZh ? '客观量化，消除主观判断' : 'Objective and quantified, eliminates subjective judgment'}</li>
              </ul>
            </div>
            <div className="p-6 bg-red-50 dark:bg-red-900/20 border-2 border-red-600">
              <h3 className="text-lg font-bold mb-3 text-white">
              ...
              <ul className="list-disc pl-6 space-y-2 text-sm">
                <li className="text-gray-300">{isZh ? '仍然忽略市场结构（支撑/阻力位）' : 'Still ignores market structure (support/resistance)'}</li>
                <li className="text-gray-300">{isZh ? '波动剧增时，ATR值滞后，止损可能过大' : 'During volatility spike, ATR lags, stop may be too wide'}</li>
                <li className="text-gray-300">{isZh ? '需要计算器或工具，手动计算较复杂' : 'Needs calculator or tools, manual calculation complex'}</li>
                <li className="text-gray-300">{isZh ? 'ATR周期选择（14/20/50）影响结果，需要优化' : 'ATR period choice (14/20/50) affects results, needs optimization'}</li>
              </ul>
            </div>
          </div>
          ...
          <div className="p-6 bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-600">
            ...
            <ul className="space-y-2 text-sm">
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '1. 结合技术位优化：' : '1. Combine with technical levels:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '2. 新闻期间加大倍数：' : '2. Increase multiplier during news:'}</strong>{' '}
                {isZh
              ...
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '3. 定期更新ATR值：' : '3. Regularly update ATR value:'}</strong>{' '}
                {isZh
                  ? 'ATR是动态指标，开仓时计算一次即可，无需持续调整止损。'
                  : 'ATR is dynamic indicator, calculate once when opening, no need to continuously adjust stop.'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '4. 回测优化倍数：' : '4. Backtest to optimize multiplier:'}</strong>{' '}
                {isZh
