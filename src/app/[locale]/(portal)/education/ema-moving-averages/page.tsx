    ...
    <EducationPageTemplate content={content} language={lang}>
      ...
      <section id="what-is-ema" className="mb-16 scroll-mt-20">
        ...
        <div className="space-y-4 text-gray-300 dark:text-gray-300 leading-relaxed">
          ...
          <div className="p-6 bg-gray-900 text-gray-300 border-2 border-gray-800">
            <h3 className="text-xl font-bold mb-4 text-white">{isZh ? 'EMA的核心特点' : 'EMA Core Features'}</h3>
            <ul className="space-y-2">
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '指数加权：' : 'Exponential weighting:'}</strong>{' '}
                {isZh
                  ? '最新价格权重最大，权重随时间指数递减'
                  : 'Latest prices have greatest weight, weight decays exponentially over time'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '反应迅速：' : 'Quick response:'}</strong>{' '}
                {isZh
                  ? '比SMA更快捕捉趋势转折点'
                  : 'Captures trend turning points faster than SMA'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '趋势跟踪：' : 'Trend following:'}</strong>{' '}
                {isZh
                  ? '优秀的动态支撑阻力位'
                  : 'Excellent dynamic support/resistance levels'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '广泛应用：' : 'Wide application:'}</strong>{' '}
                {isZh
                  ? '是MACD、布林带等众多指标的计算基础'
          ...
          <div className="p-6 bg-gray-50 dark:bg-[#0a0a0a] border-2 border-gray-800 dark:border-gray-700">
            ...
            <ul className="list-disc pl-6 space-y-2">
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '趋势识别：' : 'Trend identification:'}</strong>{' '}
                {isZh ? '价格在EMA上方=上升趋势，下方=下降趋势' : 'Price above EMA = uptrend, below = downtrend'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '交叉信号：' : 'Crossover signals:'}</strong>{' '}
                {isZh ? '快速EMA穿越慢速EMA产生买卖信号' : 'Fast EMA crossing slow EMA generates buy/sell signals'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '动态支撑阻力：' : 'Dynamic support/resistance:'}</strong>{' '}
                {isZh ? 'EMA作为价格回调的支撑或反弹的阻力' : 'EMA acts as support on pullbacks or resistance on bounces'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '止损设置：' : 'Stop loss placement:'}</strong>{' '}
      ...
      <section id="ema-vs-sma" className="mb-16 scroll-mt-20">
        ...
        <div className="space-y-6 text-gray-300 dark:text-gray-300 leading-relaxed">
            ...
            <table className="w-full border-2 border-gray-800">
              <thead>
                <tr className="bg-gray-900 text-gray-300">
                  <th className="border-2 border-gray-800 p-3 text-left">
                    {isZh ? '特征' : 'Feature'}
                  </th>
                  <th className="border-2 border-gray-800 p-3 text-left">
              ...
              <tbody className="text-gray-300 dark:text-gray-300">
                <tr>
                  <td className="border-2 border-gray-800 p-3 font-bold text-white">
          ...
          <div className="p-6 bg-gray-900 text-gray-300 border-2 border-gray-800">
            <h3 className="text-xl font-bold mb-4 text-white">{isZh ? '形象对比' : 'Visual Comparison'}</h3>
            <p className="mb-3">
              {isZh
                ? '想象一辆汽车跟随路线行驶：'
            ...
            <ul className="space-y-2">
              <li className="text-gray-300">
                <strong className="text-white">EMA</strong> = {isZh ? '运动型跑车，反应灵敏，快速转向，但可能过度反应小颠簸' : 'Sports car, responsive, quick turns, but may overreact to small bumps'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">SMA</strong> = {isZh ? '豪华轿车，行驶平稳，转向平缓，不受小波动影响' : 'Luxury sedan, smooth ride, gentle turns, unaffected by small fluctuations'}
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="space-y-6 text-gray-300 dark:text-gray-300 leading-relaxed">
          <div className="p-6 bg-gray-900 text-gray-300 border-2 border-gray-800">
            ...
            <div className="space-y-3 font-mono text-sm">
              <p className="text-gray-300">
                <strong className="text-white">{isZh ? '1. 计算平滑因子（乘数）：' : '1. Calculate Smoothing Factor (Multiplier):'}</strong>
              </p>
              <p className="pl-4">
                {isZh ? '乘数 = 2 ÷ (周期 + 1)' : 'Multiplier = 2 ÷ (Period + 1)'}
              </p>
              <p className="pl-4 text-white/60 dark:text-gray-300/60">
                {isZh ? '例如：20周期EMA的乘数 = 2 ÷ 21 = 0.0952' : 'Example: 20-period EMA multiplier = 2 ÷ 21 = 0.0952'}
              </p>
              <p className="mt-3">
                <strong className="text-white">{isZh ? '2. 计算当前EMA：' : '2. Calculate Current EMA:'}</strong>
              </p>
              <p className="pl-4">
                {isZh ? 'EMA今 = (收盘价今 - EMA昨) × 乘数 + EMA昨' : 'EMA_current = (Close_current - EMA_previous) × Multiplier + EMA_previous'}
              </p>
              <p className="mt-3">
                <strong className="text-white">{isZh ? '或简化为：' : 'Or simplified as:'}</strong>
              </p>
              <p className="pl-4">
          ...
          <div className="p-6 bg-gray-100 dark:bg-[#0a0a0a] border-l-4 border-gray-800">
            ...
            <ul className="list-disc pl-6 space-y-2">
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '8/21交叉系统：' : '8/21 Crossover System:'}</strong>{' '}
                {isZh ? '短线交易，信号快速但需严格止损' : 'Short-term trading, fast signals but need strict stops'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '21/55交叉系统：' : '21/55 Crossover System:'}</strong>{' '}
                {isZh ? '波段交易，平衡性最佳，推荐新手使用' : 'Swing trading, best balance, recommended for beginners'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '50/100/200三均线系统：' : '50/100/200 Triple MA System:'}</strong>{' '}
                {isZh ? '趋势判断，多头排列=强上升趋势' : 'Trend judgment, bullish alignment = strong uptrend'}
              </li>
              <li className="text-gray-300">
                <strong className="text-white">{isZh ? '单一200EMA：' : 'Single 200EMA:'}</strong>{' '}
        ...
        <div className="space-y-6">
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="space-y-3 text-gray-300 dark:text-gray-300">
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '21/55 EMA交叉（推荐）：' : '21/55 EMA Crossover (Recommended):'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">
                    <strong className="text-white">{isZh ? '金叉做多：' : 'Golden Cross Long:'}</strong>{' '}
                    {isZh ? '21EMA向上穿越55EMA' : '21EMA crosses above 55EMA'}
                  </li>
                  <li className="text-gray-300">{isZh ? '确认：收盘价在两条均线上方' : 'Confirm: Close above both EMAs'}</li>
                  <li className="text-gray-300">{isZh ? '入场：下一根K线开盘或回调至21EMA' : 'Entry: Next candle open or pullback to 21EMA'}</li>
                  <li className="text-gray-300">{isZh ? '止损：55EMA下方20-30点' : 'Stop: 20-30 pips below 55EMA'}</li>
                  <li className="text-gray-300">{isZh ? '止盈：风险回报比1:2-1:3' : 'Target: Risk-reward ratio 1:2-1:3'}</li>
                </ul>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a] mt-3">
                <p className="font-bold mb-2 text-white">{isZh ? '死叉做空：' : 'Death Cross Short:'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '21EMA向下穿越55EMA' : '21EMA crosses below 55EMA'}</li>
                  <li className="text-gray-300">{isZh ? '确认：收盘价在两条均线下方' : 'Confirm: Close below both EMAs'}</li>
                  <li className="text-gray-300">{isZh ? '入场：下一根K线开盘或反弹至21EMA' : 'Entry: Next candle open or bounce to 21EMA'}</li>
                  <li className="text-gray-300">{isZh ? '止损：55EMA上方20-30点' : 'Stop: 20-30 pips above 55EMA'}</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="space-y-3 text-gray-300 dark:text-gray-300">
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '上升趋势EMA回调买入：' : 'Uptrend EMA Pullback Buy:'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '确认上升趋势：价格持续在21EMA和55EMA上方' : 'Confirm uptrend: Price consistently above 21EMA and 55EMA'}</li>
                  <li className="text-gray-300">{isZh ? '等待回调：价格回落触及21EMA或55EMA' : 'Wait for pullback: Price retreats to 21EMA or 55EMA'}</li>
                  <li className="text-gray-300">{isZh ? '观察反应：在EMA处出现看涨K线形态（如Pin Bar）' : 'Observe reaction: Bullish pattern at EMA (e.g., Pin Bar)'}</li>
                  <li className="text-gray-300">{isZh ? '入场：K线收盘确认反弹后' : 'Entry: After candle close confirms bounce'}</li>
                  <li className="text-gray-300">{isZh ? '止损：EMA下方10-20点' : 'Stop: 10-20 pips below EMA'}</li>
                  <li className="text-gray-300">{isZh ? '止盈：前高或新高' : 'Target: Previous high or new high'}</li>
                </ul>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a] mt-3">
                <p className="font-bold mb-2 text-white">{isZh ? '⚠️ 注意事项：' : '⚠️ Important Notes:'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '只在明确趋势中使用此策略' : 'Only use this strategy in clear trends'}</li>
                  <li className="text-gray-300">{isZh ? '如果价格跌破关键EMA，立即止损' : 'If price breaks key EMA, stop out immediately'}</li>
                  <li className="text-gray-300">{isZh ? '首次触及EMA往往反弹，二次测试突破概率增加' : 'First EMA touch often bounces, second test has higher breakout probability'}</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 border-2 border-gray-800">
            ...
            <div className="space-y-3 text-gray-300 dark:text-gray-300">
              ...
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a]">
                <p className="font-bold mb-2 text-white">{isZh ? '使用规则：' : 'Usage Rules:'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">
                    <strong className="text-white">{isZh ? '价格在200EMA上方：' : 'Price above 200EMA:'}</strong>{' '}
                    {isZh ? '牛市环境，优先考虑做多，只做金叉多单' : 'Bull market, prioritize longs, only long on golden cross'}
                  </li>
                  <li className="text-gray-300">
                    <strong className="text-white">{isZh ? '价格在200EMA下方：' : 'Price below 200EMA:'}</strong>{' '}
                    {isZh ? '熊市环境，优先考虑做空，只做死叉空单' : 'Bear market, prioritize shorts, only short on death cross'}
                  </li>
                  <li className="text-gray-300">
                    <strong className="text-white">{isZh ? '触及200EMA：' : 'Touching 200EMA:'}</strong>{' '}
                    {isZh ? '强支撑/阻力位，首次触及往往反弹/回落' : 'Strong S/R level, first touch often bounces/retraces'}
                  </li>
                  <li className="text-gray-300">
                    <strong className="text-white">{isZh ? '突破200EMA：' : 'Breaking 200EMA:'}</strong>{' '}
                    {isZh ? '重大趋势转变信号，等待回测确认后入场' : 'Major trend change signal, wait for retest confirmation'}
                  </li>
                </ul>
              </div>
              <div className="p-4 bg-gray-50 dark:bg-[#0a0a0a] mt-3">
                <p className="font-bold mb-2 text-white">{isZh ? '实战技巧：' : 'Practical Tips:'}</p>
                <ul className="list-disc pl-6 space-y-1 text-sm">
                  <li className="text-gray-300">{isZh ? '日线和周线200EMA同时支撑/阻力 = 极强位置' : 'Daily and weekly 200EMA both supporting/resisting = extremely strong level'}</li>
                  <li className="text-gray-300">{isZh ? '价格远离200EMA时要谨慎追单，可能回归' : 'When price far from 200EMA, cautious chasing, may revert'}</li>
                  <li className="text-gray-300">{isZh ? '200EMA斜率：向上倾斜=上升趋势，向下倾斜=下降趋势' : '200EMA slope: upward = uptrend, downward = downtrend'}</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6 text-gray-300 dark:text-gray-300 leading-relaxed">
          <div className="grid md:grid-cols-2 gap-6">
            <div className="p-6 border-2 border-gray-800">
              ...
              <ul className="list-disc pl-6 space-y-1 text-sm">
                <li className="text-gray-300">{isZh ? '21EMA处的Pin Bar = 强反转信号' : 'Pin Bar at 21EMA = strong reversal'}</li>
                <li className="text-gray-300">{isZh ? '55EMA处的看涨吞没 = 趋势延续确认' : 'Bullish engulfing at 55EMA = trend continuation confirmation'}</li>
                <li className="text-gray-300">{isZh ? 'EMA金叉 + 突破关键阻力 = 最强入场点' : 'EMA golden cross + break key resistance = strongest entry'}</li>
              </ul>
            </div>
            ...
            <div className="p-6 border-2 border-gray-800">
              ...
              <ul className="list-disc pl-6 space-y-1 text-sm">
                <li className="text-gray-300">{isZh ? 'EMA金叉 + MACD金叉 = 双重确认做多' : 'EMA golden cross + MACD golden cross = double confirmation long'}</li>
                <li className="text-gray-300">{isZh ? '价格在200EMA上 + MACD零轴上金叉 = 最强趋势' : 'Price above 200EMA + MACD golden cross above zero = strongest trend'}</li>
                <li className="text-gray-300">{isZh ? 'EMA背离 + MACD背离 = 极强反转警告' : 'EMA divergence + MACD divergence = extremely strong reversal warning'}</li>
              </ul>
            </div>
            ...
            <div className="p-6 border-2 border-gray-800">
              ...
              <ul className="list-disc pl-6 space-y-1 text-sm">
                <li className="text-gray-300">{isZh ? '做多时：止损随21EMA上移' : 'In longs: Stop trails below 21EMA'}</li>
                <li className="text-gray-300">{isZh ? '波段持仓：使用55EMA或200EMA' : 'Swing positions: Use 55EMA or 200EMA'}</li>
                <li className="text-gray-300">{isZh ? '收盘价跌破EMA才止损，避免短期波动' : 'Only stop on close below EMA, avoid short-term noise'}</li>
              </ul>
            </div>
            ...
            <div className="p-6 border-2 border-gray-800">
              ...
              <ul className="list-disc pl-6 space-y-1 text-sm">
                <li className="text-gray-300">{isZh ? '日线：200EMA确定整体趋势方向' : 'Daily: 200EMA determines overall trend direction'}</li>
                <li className="text-gray-300">{isZh ? '4小时：21/55EMA寻找入场时机' : '4H: 21/55EMA finds entry timing'}</li>
                <li className="text-gray-300">{isZh ? '1小时：8/21EMA精确入场点' : '1H: 8/21EMA pinpoints entry'}</li>
                <li className="text-gray-300">{isZh ? '三个周期EMA均多头排列 = 最强趋势' : 'All three timeframes bullish alignment = strongest trend'}</li>
              </ul>
            </div>
          </div>
        </div>
